...
以下のサンプルはそれぞれ異なる原画像を使い、Panel 147 が一枚ずつ独立した一回の生成で作成しました。AIメタデータは削除済みです。横長は左に実写、右にデザインを置く厳密な50:50、縦長は上に実写、下にデザインを置く厳密な50:50です。
...
サンプルには原写真に即した短い英語コピーと、原始スタイルに沿った日本語の商品ラベルを使用しています。包装情報はデザイン上の例示です。
...
- `design-only`：全画面を Panel 147 のデザイン翻訳にし、写真は見えない参照にします。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-147` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-147 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-147-prompt.en.md)／[中国語](references/xxd-panel-147-prompt.zh-CN.md)を参照してください。
